"""Build COV-CTR validation artifacts under a configured writable directory."""
...
from .validate_cov_ctr import validate_cov_ctr
...
def build_cov_ctr_manifest(
    source_path: str | Path,
    processed_root: str | Path,
    expected: Mapping[str, int] | None = None,
) -> ManifestArtifacts:
    validation = validate_cov_ctr(str(source_path), expected=expected)
    output = Path(processed_root) / "cov_ctr"
    audit_path = output / "audit_manifest.jsonl"
    valid_path = output / "valid_samples.jsonl"
    summary_path = output / "validation_summary.json"

    audit_hash = write_jsonl(validation.audit_records, audit_path)
    valid_hash = write_jsonl(validation.valid_samples, valid_path)
